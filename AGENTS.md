...
- **Input meter = trimmed signal:** the published `inputLevel` is the **trimmed** RMS NoNoise actually processes, not the raw source level. `captureOutput` measures raw + trimmed in one allocation-free pass via `SmartLevelController.applyInputVolumeAndMeasure`, and `publishMeterTelemetry` derives the input-side fields (`inputLevel`, `isInputNearCeiling`, `isSourceMicClipping`, `consecutiveTrimmedHotTicks`) from `SmartLevelController.evaluateInputGuard`. Raw peak still drives the source-clip warning separately.
